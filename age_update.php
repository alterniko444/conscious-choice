...
<body>
    ...
    <div class="two">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <p class="hey3">Enter new age</p>
            <input type="text" class="heyb1" name="age" required>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $stmt = $conn->prepare("UPDATE user_details SET age=? WHERE user_id = ?");
            $stmt->bind_param("ss", $newAge, $username);
            $stmt->execute();
            
            echo "<script>alert('Age updated successfully!');window.location.href='user_profile.php'</script>";
