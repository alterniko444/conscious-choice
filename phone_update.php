...
<body>
    ...
    <div class="two">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <p class="hey3">Enter new phone number</p>
            <input type="text" class="heyb1" name="phone" required>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $stmt = $conn->prepare("UPDATE user_details SET phoneno=? WHERE user_id = ?");
            $stmt->bind_param("ss", $newPhone, $username);
            $stmt->execute();
            $stmt->close();
            $conn->close();
            echo "<script>alert('Phone number updated successfully!');window.location.href='user_profile.php';</script>";
        }
        ?>
    </div>
</body>
